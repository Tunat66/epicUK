                             ...
                             TString eicrecon = "v1.14.0") // name = p, pt for getting p or pt dependence fitted results
{
   ...
   std::vector<double> dcaTresolV_real, err_dcaTresolV_real;
   momV_real.clear(); dcaTresolV_real.clear(); err_dcaTresolV_real.clear();
   ...
   //Reading the root file
    ...
    TGraphErrors *gr_dcaT_real, *gr_dcaZ_real;
     ...
     Int_t etamin_bin = hist_d0xy_real->GetYaxis()->FindBin(etamin+0.0001);
     Int_t etamax_bin = hist_d0xy_real->GetYaxis()->FindBin(etamax-0.0001);
    ...
    Int_t ptmin_bin = hist_d0xy_real->GetZaxis()->FindBin(ptmin+0.0001);
    Int_t ptmax_bin = hist_d0xy_real->GetZaxis()->FindBin(ptmax-0.0001);
   ...
   if (histd0xy_real_1d->GetEntries() < 1) continue;    
...
void doCompare_results_dcaT(std::vector<TString> directory_list) 
{
	...
	//=== style of the plot=========
     ...
     lDCAT->SetHeader(Form("%s ePIC: %1.1f < #eta < %1.1f",symbolname.Data(),etamin,etamax),"C");
     int i = 0;
     for(auto& directory : directory_list)
	{
          ...
          mgDCAT->Add(new_graph);
		lDCAT->AddEntry(new_graph, directory);
          i = (i+1) % colors.size();
     }

     c_dcaxy->cd();
     mgDCAT->GetXaxis()->SetRangeUser(0.18,mgDCAT->GetXaxis()->GetXmax()); 
	mgDCAT->GetYaxis()->SetRangeUser(0.0,100); 
     mgDCAT->Draw("AP");
     lDCAT->Draw("same");
     draw_req_DCA(etamin,etamax,0.,mgDCAT->GetXaxis()->GetXmax());
     c_dcaxy->SaveAs(Form("dcaxy_resol_%1.1f_eta_%1.1f.png",etamin,etamax));
